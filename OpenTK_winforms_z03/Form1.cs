...
namespace OpenTK_winforms_z02
{
    ...
    {
        private bool statusTransparentObject;
        ...
        // Stări pentru sfera cu textură (100% și 50%)
        private bool statusSphereTex100;
        private bool statusSphereTex50;
        ...
        // Variabile pentru sferă
        private int sphereSlices = 30;
        private int sphereStacks = 30;
        private float sphereRadius = 30.0f;
        private float spherePosX = 80.0f;
        private float spherePosY = 0.0f;
        private float spherePosZ = 0.0f;
        ...
        private void SetupValues()
        {
            ...
            statusSphereTex100 = false; // Inițializare stare sferă 100%
            statusSphereTex50 = false;  // Inițializare stare sferă 50%
            brick = true;
            colorTex = 0;
        }
        ...
        private void setTransparentObjectStatus(bool status)
        {
            statusTransparentObject = status;
            btnTransparentObject.Text = status ? "Obiect transparent ON" : "Obiect transparent OFF";
        }


        private void btnTransparentObject_Click(object sender, EventArgs e)
        {
            setTransparentObjectStatus(!statusTransparentObject);
        ...
        private void GlControl1_Paint(object sender, PaintEventArgs e)
        {
            ...
            {
                DeseneazaCubQ_Tex1();
                // Sfera cu texturare 100% pentru cubul quad 100%
                DeseneazaSferaCuTextura100();
            }
            if (statusCubeTex2 == true)
            {
                DeseneazaCubQ_Tex2();
                // Sfera cu texturare 50% pentru cubul quad 50%
                DeseneazaSferaCuTextura50();
            }
            if (statusCubeTex3 == true)
            {
                DeseneazaCubT_Tex3();
                // Sfera cu texturare 100% pentru cubul triunghiuri 100%
                DeseneazaSferaCuTextura100();
            }
            if (statusCubeTex4 == true)
            {
                DeseneazaCubT_Tex4();
                // Sfera cu texturare 50% pentru cubul triunghiuri 50%
                DeseneazaSferaCuTextura50();
            }
            if (statusTransparentObject)
            {
                DeseneazaObiectTransparent();
            }
        ...
        // Metodă pentru desenarea sferei cu textură 100%
        private void DeseneazaSferaCuTextura100()
        {
            // Aplică aceeași textură ca la cub
            if (brick == true)
            {
            ...
            // Aplică aceeași culoare ca la cub
            switch (colorTex)
            {
                case 0:
            ...
            // Translatează sfera în poziția dorită (lângă cub)
            GL.Translate(spherePosX, spherePosY, spherePosZ);

            // Desenează sfera cu texturare 100%
            GL.Begin(PrimitiveType.TriangleStrip);

            for (int i = 0; i <= sphereStacks; i++)
            {
                double lat0 = Math.PI * (-0.5 + (double)(i - 1) / sphereStacks);
                double z0 = Math.Sin(lat0);
                double zr0 = Math.Cos(lat0);

                double lat1 = Math.PI * (-0.5 + (double)i / sphereStacks);
                double z1 = Math.Sin(lat1);
                double zr1 = Math.Cos(lat1);

                for (int j = 0; j <= sphereSlices; j++)
                {
                    double lng = 2 * Math.PI * (double)(j - 1) / sphereSlices;
                    double x = Math.Cos(lng);
                    double y = Math.Sin(lng);

                    // Normală și coordonată de textură pentru primul punct (100%)
                    GL.Normal3(x * zr0, y * zr0, z0);
                    GL.TexCoord2((double)j / sphereSlices, (double)(i - 1) / sphereStacks);
                    GL.Vertex3(x * zr0 * sphereRadius, y * zr0 * sphereRadius, z0 * sphereRadius);

                    // Normală și coordonată de textură pentru al doilea punct (100%)
                    GL.Normal3(x * zr1, y * zr1, z1);
                    GL.TexCoord2((double)j / sphereSlices, (double)i / sphereStacks);
                    GL.Vertex3(x * zr1 * sphereRadius, y * zr1 * sphereRadius, z1 * sphereRadius);
                }
            }

            GL.End();

            // Restaurează matricea
            GL.PopMatrix();
        }

        // Metodă pentru desenarea sferei cu textură 50%
        private void DeseneazaSferaCuTextura50()
        {
            // Aplică aceeași textură ca la cub
            if (brick == true)
            {
            ...
            // Desenează sfera cu texturare 50%
            GL.Begin(PrimitiveType.TriangleStrip);
            ...
            for (int i = 0; i <= sphereStacks; i++)
            {
                ...
                for (int j = 0; j <= sphereSlices; j++)
                {
                    ...
                    // Normală și coordonată de textură pentru primul punct (50% - scalată)
                    GL.Normal3(x * zr0, y * zr0, z0);
                    // Coordonate de textură scalate la 50%
                    GL.TexCoord2(((double)j / sphereSlices) * 0.5, ((double)(i - 1) / sphereStacks) * 0.5);
                    GL.Vertex3(x * zr0 * sphereRadius, y * zr0 * sphereRadius, z0 * sphereRadius);

                    // Normală și coordonată de textură pentru al doilea punct (50% - scalată)
                    GL.Normal3(x * zr1, y * zr1, z1);
                    // Coordonate de textură scalate la 50%
                    GL.TexCoord2(((double)j / sphereSlices) * 0.5, ((double)i / sphereStacks) * 0.5);
                    GL.Vertex3(x * zr1 * sphereRadius, y * zr1 * sphereRadius, z1 * sphereRadius);
                }
            }
        ...
        private void DeseneazaObiectTransparent()
        {
            ...
            GL.Color4(0.75f, 0.0f, 0.0f, 0.5f);

            GL.Begin(PrimitiveType.Quads);


            float planSize = 40.0f;
            float planZ = 30.0f;

            GL.Vertex3(-planSize, -planSize, planZ);
            GL.Vertex3(planSize, -planSize, planZ);
            GL.Vertex3(planSize, planSize, planZ);
            GL.Vertex3(-planSize, planSize, planZ);
        ...
        private void btnTexT_1_Click(object sender, EventArgs e)
        {
            ...
            loadTList();
            statusCubeTex3 = true;
            // Sfera va fi desenată automat în GlControl1_Paint
            GlControl1.Invalidate();
        }
        ...
        private void resetTexCubes()
        {
            ...
            statusCubeTex3 = false;
            statusCubeTex4 = false;
            statusSphereTex100 = false; // Dezactivează sfera 100%
            statusSphereTex50 = false;  // Dezactivează sfera 50%
            GlControl1.Invalidate();
        }
